...
class Hs100Platform {
  constructor (log, config, api) {
    ...
    this.config.deviceTypes = this.config.deviceTypes || ['plug'];
    this.config.addCustomCharacteristics = this.config.addCustomCharacteristics || true;

    this.config.deviceOptions = this.config.deviceOptions || {};
    this.config.deviceOptions.discoveryInterval = this.config.deviceOptions.discoveryInterval || this.config.pollingInterval * 1000 || 10000;
    this.config.deviceOptions.memoize = this.config.deviceOptions.memoize || this.config.cacheTtl * 1000 || null;

    this.homebridgeAccessories = new Map();
    this.deviceAccessories = new Map();

    let Hs100Log = Object.assign({}, this.log, {prefix: this.log.prefix + '.' + 'API'});

    this.client = new Hs100Api.Client({ logger: Hs100Log });
    ...
    this.api.on('didFinishLaunching', () => {
      ...
      this.client.startDiscovery({deviceTypes: this.config.deviceTypes, deviceOptions: this.config.deviceOptions});
    });
  }
