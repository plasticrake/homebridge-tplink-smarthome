...
class Hs100Platform {
  constructor (log, config, api) {
    ...
    this.client.on('device-new', (device) => {
      this.log.info('New Device Online: [%s] %s [%s]', device.name, device.type, device.deviceId, device.host, device.port);
      this.addAccessory(device);
    });

    this.client.on('device-online', (device) => {
      this.log.debug('Device Online: [%s] %s [%s]', device.name, device.type, device.deviceId, device.host, device.port);
      this.addAccessory(device);
    });
    ...
    this.client.on('device-offline', (device) => {
      ...
      if (deviceAccessory !== undefined) {
        this.log.debug('Device Offline: [%s] %s [%s]', deviceAccessory.homebridgeAccessory.displayName, device.type, device.deviceId, device.host, device.port);
      }
    });
