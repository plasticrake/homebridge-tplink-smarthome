...
const configSetup = function () {
  ...
  dis.excludeMacAddresses = dis.excludeMacAddresses || c.excludeMacAddresses || [];
  ...
  dev.defaultSendOptions = dev.defaultSendOptions || Object.assign({}, dso);
...
class TplinkSmarthomePlatform {
  constructor (log, config, homebridge) {
    ...
    this.customCharacteristics = require('./characteristics')(homebridge.hap.Characteristic);

    this.log.info(
      '%s v%s, node %s, homebridge v%s',
      packageConfig.name, packageConfig.version, process.version, homebridge.serverVersion
    );
    ...
    this.client = new Client({ logger: TplinkSmarthomeLog, defaultSendOptions: this.config.defaultSendOptions });
    ...
    this.getCategoryName = lookup.bind(homebridge.hap.Accessory.Categories, null);
    this.getServiceName = lookup.bind(
      homebridge.hap.Service, (thisKeyValue, value) => thisKeyValue.UUID === value.UUID
    );

    this.getCharacteristicName = (characteristic) => {
      return characteristic.name || characteristic.displayName ||
        lookup.bind(
          homebridge.hap.Characteristic, (thisKeyValue, value) => thisKeyValue.UUID === value.UUID
        )(characteristic);
    };
  }
  ...
  registerPlatformAccessory (platformAccessory) {
    ...
    this.homebridge.registerPlatformAccessories(
      'homebridge-tplink-smarthome', 'TplinkSmarthome', [platformAccessory]
    );
  }
  ...
  removeAccessory (homebridgeAccessory) {
    ...
    this.homebridge.unregisterPlatformAccessories(
      'homebridge-tplink-smarthome', 'TplinkSmarthome', [homebridgeAccessory]
    );
  }
}

function createTplinkAccessory (platform, accessory, tplinkDevice) {
  const config = platform.config;
  const Categories = platform.homebridge.hap.Accessory.Categories;
  ...
  return new TplinkAccessory(platform, platform.config, accessory, tplinkDevice, category, services);
}
