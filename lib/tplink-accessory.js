...
class TplinkAccessory {
  constructor (platform, config, homebridgeAccessory, tplinkDevice, category, services) {
    ...
    if (!this.homebridgeAccessory) {
      ...
      this.log.debug(`[${this.name}] Creating new Accessory [${this.hkDevice.id}] [${uuid}] category: ${categoryName || ''}`);
      this.homebridgeAccessory = new PlatformAccessory(this.name, uuid, category);
      this.platform.registerPlatformAccessory(this.homebridgeAccessory);
    } else {
      this.log.debug(`[${this.name}] Existing Accessory found [${homebridgeAccessory.context.deviceId}] [${homebridgeAccessory.UUID}] category: ${categoryName || ''}`);
      this.homebridgeAccessory.displayName = this.name;
      if (this.homebridgeAccessory.category !== category) {
        this.log.warn(`[${this.name}] Correcting Accessory Category from: ${platform.getCategoryName(this.homebridgeAccessory.category)} to: ${categoryName || ''}`);
        this.homebridgeAccessory.category = category;
      }
    }

    this.services['AccessoryInformation'] = AccessoryInformation(platform.homebridge.hap)(this.homebridgeAccessory, this.hkDevice);

    this.homebridgeAccessory.context.deviceId = this.hkDevice.id;
    this.homebridgeAccessory.on('identify', this.hkDevice.identify);
    ...
    if (this.services['Outlet']) {
      this.setupCharacteristics(this.services['Outlet'], [
        Characteristic.Name, Characteristic.On, Characteristic.OutletInUse,
        CustomCharacteristic.Amperes, CustomCharacteristic.KilowattHours,
        CustomCharacteristic.VoltAmperes, CustomCharacteristic.Volts, CustomCharacteristic.Watts
      ]);
    }

    if (this.services['Switch']) {
      this.setupCharacteristics(this.services['Switch'], [
        Characteristic.Name, Characteristic.On
      ]);
    }

    if (this.services['Lightbulb']) {
      this.setupCharacteristics(this.services['Lightbulb'], [
        Characteristic.Name, Characteristic.On, Characteristic.Brightness,
        Characteristic.ColorTemperature, Characteristic.Hue, Characteristic.Saturation,
    ...
    this.homebridgeAccessory.services.forEach((service) => {
      ...
      this.log.warn(`[${this.name}] Removing stale Service: [%s] uuid:[%s] subtype:[%s]`, this.platform.getServiceName(service), service.UUID, service.subtype || '');
  ...
  setupCharacteristics (service, characteristics) {
    const characteristicsToRemove = service.characteristics.filter((existingCharacteristic) => {
      return !characteristics.find((characteristicToSetup) => characteristicToSetup.UUID === existingCharacteristic.UUID);
    });

    characteristicsToRemove.forEach((characteristicToRemove) => {
      this.log.warn(`[${this.name}] Removing stale Characteristic: [%s.%s] uuid:[%s]`, this.platform.getServiceName(service), this.platform.getCharacteristicName(characteristicToRemove), characteristicToRemove.UUID);
      service.removeCharacteristic(characteristicToRemove);
    });

    characteristics.forEach((characteristic) => {
      if (this.hkDevice.supportsCharacteristic(characteristic)) {
        let c = getOrAddCharacteristic(service, characteristic);
        c.on('get', this.callbackify(() => { return this.hkDevice.getCharacteristicValue(characteristic); }));

        if (this.hkDevice.supportsCharacteristicSet(characteristic)) {
          c.on('set', this.callbackify((value) => { return this.hkDevice.setCharacteristicValue(characteristic, value); }));
        }
