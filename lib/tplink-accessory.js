...
class TplinkAccessory {
  constructor (platform, config, homebridgeAccessory, tplinkDevice, category, services) {
    ...
    this.homebridgeAccessory.on('identify', (...args) => { this.hkDevice.identify(...args); });
    ...
    if (this.services['Outlet']) {
      let characteristics = [Characteristic.Name, Characteristic.On, Characteristic.OutletInUse];

      if (this.config.addCustomCharacteristics) {
        characteristics.push(...[CustomCharacteristic.Amperes, CustomCharacteristic.KilowattHours,
