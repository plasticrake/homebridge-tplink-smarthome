  ...
  constructor (platform, config, homebridgeAccessory, plug) {
    ...
    if (plug.supportsEmeter && this.config.addCustomCharacteristics) {
      ...
      getOrAddCharacteristic(this.outletService, CustomCharacteristic.Volts)
        .on('get', (callback) => {
          this.plug.getEmeterRealtime().then((emeterRealtime) => {
            callback(null, Math.round(emeterRealtime.voltage));
          }).catch((reason) => {
            this.log.error('[%s] %s', this.homebridgeAccessory.displayName, 'Volts getEmeterRealtime');
            this.log.error(reason);
            callback(new Error(reason));
          });
        });
      ...
      getOrAddCharacteristic(this.outletService, CustomCharacteristic.Amperes)
        .on('get', (callback) => {
          this.plug.getEmeterRealtime().then((emeterRealtime) => {
            ...
            this.log.error('[%s] %s', this.homebridgeAccessory.displayName, 'Amperes getEmeterRealtime');
            this.log.error(reason);
            callback(new Error(reason));
          });
        });
      ...
      getOrAddCharacteristic(this.outletService, CustomCharacteristic.Watts)
        .on('get', (callback) => {
          this.plug.getEmeterRealtime().then((emeterRealtime) => {
            ...
            this.log.error('[%s] %s', this.homebridgeAccessory.displayName, 'Watts getEmeterRealtime');
            this.log.error(reason);
            callback(new Error(reason));
          });
        });
      ...
      getOrAddCharacteristic(this.outletService, CustomCharacteristic.VoltAmperes)
        .on('get', (callback) => {
          this.plug.getEmeterRealtime().then((emeterRealtime) => {
            ...
            this.log.error('[%s] %s', this.homebridgeAccessory.displayName, 'VoltAmperes getEmeterRealtime');
            this.log.error(reason);
            callback(new Error(reason));
          });
        });
      ...
      getOrAddCharacteristic(this.outletService, CustomCharacteristic.KilowattHours)
        .on('get', (callback) => {
          this.plug.getEmeterRealtime().then((emeterRealtime) => {
            ...
            this.log.error('[%s] %s', this.homebridgeAccessory.displayName, 'KilowattHours getEmeterRealtime');
            this.log.error(reason);
            callback(new Error(reason));
          });
        });
