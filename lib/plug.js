...
const { callbackify, getOrAddCharacteristic, lookup, removeCharacteristicIfFound } = require('./utils');
...
class PlugAccessory {
  constructor (platform, config, homebridgeAccessory, plug) {
    ...
    const CustomCharacteristic = require('./homekit')(platform.api);
    ...
    if (plug.supportsEmeter && this.config.addCustomCharacteristics) {
      ...
      getOrAddCharacteristic(this.outletService, CustomCharacteristic.Volts)
        .on('get', callbackify(this.getVolts.bind(this), this.logErr('Volts')));

      getOrAddCharacteristic(this.outletService, CustomCharacteristic.Amperes)
        .on('get', callbackify(this.getAmperes.bind(this), this.logErr('Amperes')));

      getOrAddCharacteristic(this.outletService, CustomCharacteristic.Watts)
        .on('get', callbackify(this.getWatts.bind(this), this.logErr('VoltAmperes')));

      getOrAddCharacteristic(this.outletService, CustomCharacteristic.VoltAmperes)
        .on('get', callbackify(this.getVoltAmperes.bind(this), this.logErr('VoltAmperes')));

      getOrAddCharacteristic(this.outletService, CustomCharacteristic.KilowattHours)
        .on('get', callbackify(this.getKilowattHours.bind(this), this.logErr('KilowattHours')));
    } else {
