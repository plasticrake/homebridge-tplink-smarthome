...
class PlugAccessory {
  constructor (platform, config, homebridgeAccessory, plug) {
    ...
    if (!this.homebridgeAccessory) {
      this.log.debug('Creating new Accessory [%s] [%s] [%s] homekitCategory: %s', plug.alias, plug.id, UUIDGen.generate(plug.id), this.homekitCategory);
      this.homebridgeAccessory = new PlatformAccessory(plug.alias, UUIDGen.generate(plug.id), Accessory.Categories[this.homekitCategory]);
    ...
    if (plug.supportsEmeter && this.config.addCustomCharacteristics) {
      ...
      getOrAddCharacteristic(this.outletService, CustomCharacteristic.Volts)
        .on('get', (callback) => {
          this.plug.emeter.getRealtime().then((emeterRealtime) => {
            callback(null, emeterRealtime.voltage);
          }).catch((reason) => {
      ...
      getOrAddCharacteristic(this.outletService, CustomCharacteristic.Amperes)
        .on('get', (callback) => {
          this.plug.emeter.getRealtime().then((emeterRealtime) => {
            callback(null, emeterRealtime.current);
          }).catch((reason) => {
      ...
      getOrAddCharacteristic(this.outletService, CustomCharacteristic.Watts)
        .on('get', (callback) => {
          this.plug.emeter.getRealtime().then((emeterRealtime) => {
            callback(null, emeterRealtime.power);
          }).catch((reason) => {
      ...
      getOrAddCharacteristic(this.outletService, CustomCharacteristic.VoltAmperes)
        .on('get', (callback) => {
          this.plug.emeter.getRealtime().then((emeterRealtime) => {
            callback(null, emeterRealtime.voltage * emeterRealtime.current);
          }).catch((reason) => {
      ...
      getOrAddCharacteristic(this.outletService, CustomCharacteristic.KilowattHours)
        .on('get', (callback) => {
          this.plug.emeter.getRealtime().then((emeterRealtime) => {
            callback(null, emeterRealtime.total);
          }).catch((reason) => {
